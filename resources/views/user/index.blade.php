            ...
            <section class="my-5 flex flex-col gap-5">
                @foreach ($categories as $category)
                    <div class="flex flex-wrap md:flex-nowrap h-fit hover:shadow-md rounded-md justify-between items-center p-3 py-5 relative gap-2">
                        ...
                        <a href="{{ route('categories.edit',$category) }}" class="btn bg-app-blue rounded self-center block w-full md:max-w-max-content z-10 text-[white] p-2 hover:!shadow-lg font-semibold"> Edit
                        </a>
                        @endif
                    </div>
                @endforeach
